...
const getTransporter = async () => {
    try {
        ...
        if (!token) throw new Error("[EmailService] ❌ Failed to acquire OAuth2 Access Token.");
...
export const sendCandidateEmail = async (
  ...
  companyName: string = "HireSphere Recruitment",
  actionUrl?: string
) => {
  try {
...
export const sendInterviewInvitation = async (
  ...
  type: 'candidate' | 'interviewer' = 'candidate',
  companyName: string = "HireSphere Recruitment"
) => {
  try {
 ...
 */
export const sendEvaluationReport = async (
  to: string,
  candidateName: string,
