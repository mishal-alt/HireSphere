...
export const getAnalytics = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const totalHires = await Candidate.countDocuments({ 
            companyId, 
            status: { $in: ["Hired", "Hired (Signed)"] } 
        });

        const activePipeline = await Candidate.countDocuments({
            companyId,
            status: { $in: ["Shortlisted", "Scheduled", "Interviewed", "Offered"] }
        });

        const totalOffered = await Candidate.countDocuments({
            companyId,
        ...
        const velocityData = await Candidate.aggregate([
            { 
                $match: { 
                    companyId, 
                    status: { $in: ["Hired", "Hired (Signed)"] },
        ...
        let deptEfficiency = await Candidate.aggregate([
            { $match: { companyId } },
            {
                $lookup: {
                    from: 'jobs',
        ...
        const sourcesData = await Candidate.aggregate([
            { $match: { companyId } },
            {
                $group: {
        ...
        // 5. Top Interviewers
        let topInterviewersData = await Interview.aggregate([
